...
void botMove()
{
    ...
    // Жадный алгоритм без заглядывания вперёд
    bestScore = -100;
    bestMoveId = 0;

    for (char i = 0; i < moveCount; i++)
    {
        x1 = moves[i * 4] - 'a';
        y1 = moves[i * 4 + 1] - '1';
        x2 = moves[i * 4 + 2] - 'a';
        ...
        // Взятие всегда лучше обычного хода
        if (ABS(x2 - x1) == 2)
            score += 10;

        // Движение к дамке
        if ((y2 == 0 || y2 == 7) && !checkerIsKing[TABLE_INDEX[y1][x1]])
            score += 5;
        ...
        // Проверка для простых шашек противника (сверху вниз)
        // Проверяем левую-верхнюю диагональ
        if (x2 - 1 >= 0 && y2 - 1 >= 0)
        {
            char id = TABLE_INDEX[y2 - 1][x2 - 1];
            if (id >= 0 && id < 12)
            { // Если там черная шашка
                // Проверяем есть ли место для прыжка
                if (x2 + 1 < 8 && y2 + 1 < 8 && TABLE_INDEX[y2 + 1][x2 + 1] == -1)
                {
                    inDanger = 1;
                }
            }
        ...
        if (!inDanger && x2 + 1 < 8 && y2 - 1 >= 0)
        {
            id = TABLE_INDEX[y2 - 1][x2 + 1];
            if (id >= 0 && id < 12)
            {
                if (x2 - 1 >= 0 && y2 + 1 < 8 && TABLE_INDEX[y2 + 1][x2 - 1] == -1)
                {
                    inDanger = 1;
                }
            }
        ...
        // Проверка для дамок противника (снизу вверх)
        if (!inDanger && x2 - 1 >= 0 && y2 + 1 < 8)
        {
            id = TABLE_INDEX[y2 + 1][x2 - 1];
            if (id >= 0 && id < 12 && checkerIsKing[id])
            { // Если там черная дамка
                // Проверяем есть ли место для прыжка
                if (x2 + 1 < 8 && y2 - 1 >= 0 && TABLE_INDEX[y2 - 1][x2 + 1] == -1)
                {
                    inDanger = 1;
                }
            }
        ...
        if (!inDanger && x2 + 1 < 8 && y2 + 1 < 8)
        {
            char id = TABLE_INDEX[y2 + 1][x2 + 1];
            if (id >= 0 && id < 12 && checkerIsKing[id])
            {
                if (x2 - 1 >= 0 && y2 - 1 >= 0 && TABLE_INDEX[y2 - 1][x2 - 1] == -1)
                {
                    inDanger = 1;
                }
            }
    ...
    x1 = moves[bestMoveId * 4] - 'a';
    y1 = moves[bestMoveId * 4 + 1] - '1';
    x2 = moves[bestMoveId * 4 + 2] - 'a';
    y2 = moves[bestMoveId * 4 + 3] - '1';
